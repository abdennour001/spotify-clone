 ...
 */
export default function UrlSearchParams(url) {
        ...
        for (var i = 0; i < entries.length; i++) {
            ...
            if (this.has(entry[0])) {
                paramsString +=
                    ...
                    entry[0] +
                    "=" +
                    encodeURIComponent(this._unesc(entry[1]));
            }
        }
    ...
    if (paramPairs.indexOf("#") > -1) {
        urlHash = paramPairs.substr(
            paramPairs.indexOf("#") + 1,
            paramPairs.length
        );
    ...
    for (var i = 0; i < paramPairs.length; i++) {
        ...
        if (paramComponents.length === 2) {
            urlParamMap[paramComponents[0]] = this._esc(
                decodeURIComponent(paramComponents[1])
            );
        }
    }
}
